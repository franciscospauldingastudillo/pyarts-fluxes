...
def create_gas_scattering_agendas_in_WS(ws):
    ...
    gas_scattering_agenda_list = ['gas_scattering_agenda__Rayleigh', 'gas_scattering_agenda__IsotropicConstant']
...
def create_surface_agendas_in_WS(ws):
    ...
    iy_surface_agenda_list = ['iy_surface_agenda__FlatUserReflectivity', 'iy_surface_agenda__FlatFresnel', 'iy_surface_agenda_Lambertian']
...
def create_pnd_agendas_SB06_in_WS(ws):
    ...
    pnd_agenda_list = ['pnd_agenda_SB06LWC', 'pnd_agenda_SB06IWC', 'pnd_agenda_SB06RWC', 'pnd_agenda_SB06SWC', 'pnd_agenda_SB06GWC', 'pnd_agenda_SB06HWC']  
...
def pnd_agenda_SB06LWC(ws):
    ws.ExtractFromMetaSingleScatSpecies(ws.pnd_size_grid, ws.scat_meta,
                                        "mass", ws.agenda_array_index)
...
def create_pnd_agendas_MY05_in_WS(ws):
    ...
    pnd_agenda_list = ['pnd_agenda_MY05LWC', 'pnd_agenda_MY05IWC', 'pnd_agenda_MY05RWC', 'pnd_agenda_MY05SWC', 'pnd_agenda_MY05GWC', 'pnd_agenda_MY05HWC']  
...
def pnd_agenda_MY05LWC(ws):
    ws.ExtractFromMetaSingleScatSpecies(ws.pnd_size_grid, ws.scat_meta,
                                        "diameter_max", ws.agenda_array_index)
    ws.Copy(ws.psd_size_grid, ws.pnd_size_grid)
...
def create_pnd_agendas_CG_in_WS(ws):
    ...
    pnd_agenda_list = ['pnd_agenda_CGLWC', 'pnd_agenda_CGIWC', 'pnd_agenda_CGRWC', 'pnd_agenda_CGSWC_tropic', 'pnd_agenda_CGSWC_midlatitude', 'pnd_agenda_CGGWC']
...
def pnd_agenda_CGLWC(ws):
    ...
    ws.NumericSet(ws.scat_species_b,3.)
...
def dobatch_calc_agenda_allsky(ws):
    ...
    ws.Extract(ws.surface_scalar_reflectivity, ws.array_of_surface_scalar_reflectivity, ws.ybatch_index)
    ...
    ws.spectral_irradiance_fieldDisort(nstreams=ws.NstreamIndex, Npfct=-1,  emission=ws.EmissionIndex)
    ...
    ws.Tensor7SetConstant(ws.spectral_radiance_field, 0, 0, 0, 0, 0, 0, 0, 0.)
    ws.Tensor5SetConstant(ws.spectral_irradiance_field, 0, 0, 0, 0, 0, 0.)
...
def create_agendas_in_ws(ws, pnd_agendas=False):
    ...
    if pnd_agendas:
        ...
        pnd_agenda_list=pnd_agenda_SB06_list+pnd_agenda_MY05_list+pnd_agenda_CG_list
